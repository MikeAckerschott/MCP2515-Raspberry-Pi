...
{
    startSPI();
    bcm2835_spi_transfer(INSTRUCTION_RESET);
    endSPI();
...
{
    startSPI();
    bcm2835_spi_transfer(INSTRUCTION_READ);
    bcm2835_spi_transfer(reg);
    uint8_t ret = bcm2835_spi_transfer(0x00);
    endSPI();
...
void MCP2515::readRegisters(const REGISTER reg, uint8_t values[], const uint8_t n)
{
    ...
    for (uint8_t i=0; i<n; i++) {
        values[i] = bcm2835_spi_transfer(0x00);
    }
    endSPI();
}
...
{
    startSPI();
    bcm2835_spi_transfer(INSTRUCTION_WRITE);
    bcm2835_spi_transfer(reg);
...
{
    startSPI();
    bcm2835_spi_transfer(INSTRUCTION_BITMOD);
    bcm2835_spi_transfer(reg);
...
{
    startSPI();
    bcm2835_spi_transfer(INSTRUCTION_READ_STATUS);
    uint8_t i = bcm2835_spi_transfer(0x00);
    endSPI();
...
MCP2515::ERROR MCP2515::setMode(const CANCTRL_REQOP_MODE mode)
{
    ...
    unsigned long endTime = millisec_since_epoch + 10;
    bool modeMatch = false;
    while (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count() < endTime) {
